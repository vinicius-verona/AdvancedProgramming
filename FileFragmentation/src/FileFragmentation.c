...
/**
 * @brief Problem: 10132 - File Fragmentation
 * @author Vinicius Gabriel Angelozzi Verona de Resende
 *
 * @ignore
 * Find at:
 * https://onlinejudge.org/index.php?option=com_onlinejudge&Itemid=8&page=show_problem&problem=1073
 */
int main() {
    int cases = 0;
    scanf("%d\n", &cases);
...
int calculateNumberOfFiles(int totalFragments) { return totalFragments / 2; }
        ...
        while (problem->biggest[fi] != NULL) {
            ...
            for (int i = 0; i < ROWS; i++) {
                ...
                for (int j = 0; j < ROWS; j++) {
                    ...
                    if (i != j && problem->fragments[i]->fragmentSize + problem->fragments[j]->fragmentSize ==
                                      problem->fileSize) {
                        char npattern[300];
