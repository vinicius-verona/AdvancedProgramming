...
/**
 * @brief Problem: 10138 - CDVII
 * @author Vinicius Gabriel Angelozzi Verona de Resende
 *
 * @ignore
 * Find at: https://onlinejudge.org/index.php?option=com_onlinejudge&Itemid=8&category=32&page=show_problem&problem=1079
 */
int main() {
    // Control variables
    string stats;
    ...
    for (int i = 0; i < cases; i++) {
        ...
        while (getline(cin, line) && !line.empty()) {
            ...
            // Read records;
            Record record;
            lineStream >> record.licensePlate >> record.month >> ignore >> record.day >> ignore >> record.hour >>
                ignore >> record.minute >> stats >> record.km;
...
ostream &operator<<(ostream &os, const Record &rec) {
    os << rec.licensePlate << " - " << rec.month << "/" << rec.day << "-T-" << rec.hour << ":" << rec.minute
       << " -> KM: " << rec.km << " [" << rec.stats << "]";
    return os;
}
...
int getTime(Record record) {
    ...
    return ((record.month - 1) * monthInSeconds + (record.day - 1) * daysInSeconds + record.hour * hoursInSeconds +
    ...
    while (vehicle != records.end()) {
        ...
        for (int i = 1; i < (int)vehicleRecord.size(); i++) {
            if (vehicleRecord[i - 1].stats && !vehicleRecord[i].stats) {
                bill += FEE_PER_TRIP;
                bill += (fares[vehicleRecord[i - 1].hour] / 100) * abs(vehicleRecord[i].km - vehicleRecord[i - 1].km);
            }
        }
